...
export class SourceFileCache implements TsSourceFileContainer {
  readonly #sourceFilesByFilePath = new Map<
    StandardizedFilePath,
    ts.SourceFile
  >()
  #projectVersion = 0
  ...
  async addOrGetSourceFileFromFilePath(
  ...
  createSourceFileFromText(
    ...
    if (hasBom) text = StringUtils.stripBom(text)
  ...
  setSourceFile(sourceFile: ts.SourceFile) {
    const standardizedFilePath =
      this.#fileSystemWrapper.getStandardizedAbsolutePath(sourceFile.fileName)
